...
const STORAGE_KEY  = "leadforge_v4";
...
async function searchRealBusinesses(typeIds, count=8) {
  ...
  // Fallback to Claude if Google returns nothing
  if (results.length < 2) return await generateAILeads(typeIds, count);
  return results;
}
...
async function generateAILeads(typeIds, count=8) {
  ...
  const txt = await callClaude(
    ...
    ' [{"name":"","type":"","icon":"","address":"","city":"","employees":"2-5","hasWebsite":true,"score":8,"pain":""}]' +
    " Gebruik echte Limburgse plaatsnamen: Bergen, Gennep, Venray, Boxmeer, Venlo, Weert, Roermond, Sittard."
  );
  const clean = txt.replace(/```json|```/g,"").trim();
  const parsed = JSON.parse(clean);
  return parsed.map(l => ({
    ...l, id:crypto.randomUUID(), message:null, status:"new",
    placeId:null, rating:null, real:false, toEmail:"", phone:null,
  }));
}
...
function exportCSV(leads) {
  ...
  const rows = leads.map(l => [
    l.name, l.type, l.city, l.address, l.score, l.rating||"",
    l.phone||"", STATUSES.find(s=>s.id===l.status)?.label||"Nieuw",
    (l.message||"").replace(/\n/g," "),
  ]);
...
function LeadCard({lead, onGenerate, onPatch, onDelete, isGenerating, channel}) {
  ...
  const [sendOk, setSendOk]         = useState(null); // null|"ok"|"err"
  const [localEmail, setLocalEmail] = useState(lead.toEmail||"");

  // sync draft when message changes externally
  ...
  const handleSendEmail = async () => {
    ...
    try {
      const subject = (lead.message||"").match(/Onderwerp:\s*(.+)/)?.[1]?.trim() || "AI Automatisering voor "+lead.name;
      await sendEmailJS(localEmail.trim(), subject, lead.message||"");
      setSendOk("ok");
      onPatch(lead.id, {status:"sent", toEmail:localEmail.trim()});
    } catch(e) {
      console.error(e);
      setSendOk("err");
    }
  ...
  return (
    <div className={"relative group bg-zinc-900 border rounded-2xl p-5 flex flex-col gap-3 transition-all duration-200 "+cardBorder}>

      {/* delete */}
      <button onClick={()=>onDelete(lead.id)}
        className="absolute top-3 right-3 w-6 h-6 rounded-full bg-zinc-800 hover:bg-red-500/20 text-zinc-600 hover:text-red-400 transition opacity-0 group-hover:opacity-100 flex items-center justify-center text-base leading-none">
        ×
      </button>
      ...
      <div className="flex flex-wrap items-center gap-1.5">
        ...
        {lead.rating && <Tag cls="bg-zinc-800 text-zinc-400">★ {lead.rating} Google</Tag>}
      </div>
          ...
          <div className="flex flex-col gap-2">
            ...
            {channel==="whatsapp" ? (
              <button onClick={handleWhatsApp}
                className="w-full py-2 rounded-lg bg-green-600 hover:bg-green-500 text-white text-xs font-bold transition">
              ...
              <div className="flex flex-col gap-1.5">
                <input
                  ...
                  placeholder="E-mailadres ontvanger (bijv. info@bedrijf.nl)"
                ...
                <button onClick={handleSendEmail} disabled={sending||!localEmail.trim()}
            ...
            <div className="flex gap-2">
              ...
              <button onClick={()=>onGenerate(lead)} title="Nieuw bericht genereren"
                className="px-3 py-2 rounded-lg bg-zinc-700 hover:bg-zinc-600 text-zinc-300 text-xs transition">🔄</button>
            </div>
          </div>
        )
      ) : (
...
export default function App() {
  ...
  const doSearch = async () => {
    ...
    try {
      ...
      setError("Fout: "+e.message+". Schakel naar AI-modus als Google niet werkt.");
    }
    setLoading(false);
  };
  ...
  const doGenMsg = async (lead) => {
    ...
    const instr   = isEmail
      ? "Begin met 'Onderwerp: [pakkende titel]'. Dan 3-4 korte alineas: 1) hun specifieke situatie, 2) wat AI voor hen doet, 3) concreet voordeel (tijd/geld besparing), 4) uitnodiging voor gratis 20-min gesprek. Sluit af met contactgegevens op aparte regels."
      : "3-5 zinnen. Casual maar professioneel. Verwijs naar hun branche. Sluit af met telefoonnummer en website op een aparte regel.";
    const prompt =
      "Je bent een AI automatisering specialist in Limburg, Nederland. Bedrijf: " + bizName + "." +
      " Contactgegevens: " + CONTACT.phone + " | " + CONTACT.email + " | " + CONTACT.website + "." +
      " Schrijf een " + style + " in correct Nederlands voor: " + lead.name + " (" + lead.type + ") in " + lead.city + "." +
      " Hun pijnpunt: " + lead.pain + ". Website aanwezig: " + (lead.hasWebsite?"ja":"nee") + "." +
      " " + instr +
      " Schrijf direct en persoonlijk. Geen placeholders zoals [naam]. Sluit ALTIJD af met de echte contactgegevens.";
    try {
      const txt = await callClaude(prompt);
      patchLead(lead.id, {message:txt});
    } catch(e) {
      setError("Fout bij genereren bericht: "+e.message);
    }
    setGenMsg(null);
  };
      ...
      <div className="sticky top-0 z-20 bg-zinc-950/95 backdrop-blur border-b border-zinc-800 px-5 py-3 flex items-center justify-between gap-3">
        ...
        {screen==="leads" && (
          <div className="flex gap-2 items-center flex-wrap justify-end">
            ...
            <button onClick={()=>{setLeads([]);setScreen("setup");}}
              className="text-xs px-3 py-1.5 rounded-lg bg-zinc-800 hover:bg-zinc-700 text-zinc-400 transition">
              + Nieuw
            </button>
          </div>
        )}
        ...
        {screen==="setup" && (
          <div className="flex flex-col gap-7">
            <div>
              ...
              <p className="text-zinc-500 mt-2 text-sm leading-relaxed">Vindt echte lokale bedrijven via Google Maps en schrijft persoonlijke Dutch outreach voor jou.</p>
            </div>
            ...
            <div className="flex items-center justify-between bg-zinc-900 border border-zinc-800 rounded-xl px-4 py-3 gap-3">
              <div>
                <p className="text-sm font-semibold text-white">🗺️ Echte bedrijven via Google Maps</p>
                <p className="text-xs text-zinc-500 mt-0.5">{useReal ? "Zoekt echte Limburgse bedrijven" : "Genereert fictieve leads via AI (fallback)"}</p>
              </div>
            ...
            <div className="flex flex-col gap-3">
              ...
              <div className="flex gap-3">
                {[
                  {id:"email",    icon:"✉️",  label:"E-mail",    sub:"Verstuurt via Gmail"},
                  {id:"whatsapp", icon:"💬",  label:"WhatsApp",  sub:"Opent WhatsApp Web"},
                ].map(s=>(
